...
from easyki.easyeda.parameters_easyeda import (
    EasyedaPinType,
    EeSymbol,
    EeSymbolArc,
    EeSymbolBbox,
...
from easyki.easyeda.svg_path_parser import SvgPathEllipticalArc, SvgPathMoveTo
from easyki.core.geometry_utils import get_middle_arc_pos
from easyki.kicad.export_kicad_footprint import compute_arc
from easyki.kicad.parameters_kicad_symbol import *
...
def px_to_mil(dim: Union[int, float]) -> int:
    """
    将像素单位转换为mil（千分之一英寸）
    Convert pixel units to mil (thousandth of an inch)
    
    参数:
    Args:
        dim (Union[int, float]): 像素值 / Pixel value
        
    返回:
    Returns:
        int: 转换后的mil值 / Converted value in mil
    """
    return int(10 * dim)
...
def px_to_mm(dim: Union[int, float]) -> float:
    ...
    返回:
    Returns:
        float: 转换后的毫米值 / Converted value in millimeters
    """
...
def convert_ee_pins(
    ...
    """
    将EasyEDA引脚转换为KiCad符号引脚
    Convert EasyEDA pins to KiCad symbol pins
    
    参数:
    Args:
        ee_pins (List[EeSymbolPin]): EasyEDA引脚列表 / List of EasyEDA pins
        ee_bbox (EeSymbolBbox): EasyEDA符号边界框 / EasyEDA symbol bounding box
        kicad_version (KicadVersion): KiCad版本枚举 / KiCad version enum
        
    返回:
    Returns:
        List[KiSymbolPin]: 转换后的KiCad符号引脚列表 / List of converted KiCad symbol pins
    """
    ...
    for ee_pin in ee_pins:
        ...
        # 处理引脚样式（反相、时钟等）
        # Handle pin styles (inverted, clock, etc.)
        ...
        # 处理不同引脚长度的位置调整（已注释）
        # Handle position adjustment for different pin lengths (commented out)
...
class ExporterSymbolKicad:
    """
    KiCad符号导出器，负责将EasyEDA符号转换为KiCad符号格式
    KiCad symbol exporter, responsible for converting EasyEDA symbols to KiCad symbol format
    """

    def __init__(self, symbol, kicad_version: KicadVersion):
        """
        初始化符号导出器
        Initialize symbol exporter
        
        参数:
        Args:
            symbol: EasyEDA符号对象 / EasyEDA symbol object
            kicad_version (KicadVersion): 目标KiCad版本 / Target KiCad version
        """
    ...
    def export(self, footprint_lib_name: str) -> str:
        """
        导出符号为KiCad格式字符串
        Export symbol as KiCad format string
        
        参数:
        Args:
            footprint_lib_name (str): 封装库名称，用于设置符号的封装引用 / Footprint library name for setting symbol's footprint reference
            
        返回:
        Returns:
            str: KiCad符号格式的字符串表示 / String representation in KiCad symbol format
        """
